...
class ViewController: UIViewController {
    ...
    private func startPingingAsync() async {
        if let oldPing = ping {
            oldPing.haltPinging()
            ping = nil
        }
        ...
        var pingObj: SwiftyPing?

        do {
            pingObj = try await SwiftyPing(host: host, configuration: PingConfiguration(interval: 1.0, with: 1), queue: DispatchQueue.global())
        } catch {
            textView.text = error.localizedDescription
        }

        guard let pingObj = pingObj else { return }
        ping = pingObj
